...
| 手机侧边栏 | 原生（DSH 自身布局）/ 全屏覆盖。全屏覆盖：窄屏下内容区独占整屏，侧边栏离屏收起，从屏幕左缘向右滑动或点左侧细条把手唤起，铺满全屏显示**展开后的真实内容**（打开时自动点击 DSH 自身的展开按钮，避免显示收起态的白雾）；点遮罩、选中会话或 Esc 关闭并自动收回到 rail 态；不再遮挡头部 |
