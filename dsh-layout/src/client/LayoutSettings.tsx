      ...
      <StoreContext.Provider value={store}>
        <header className="dsh-layout-settings__header">
          <div>
            <h2>页面布局</h2>
            <p>
              材质、宽度与信息密度；每项默认保持 DSH 原生样式，只做对应的事。
            </p>
          </div>
          {overrides > 0 && (
        ...
        {tab === "material" && (
          <MaterialCard store={store} settings={settings} />
        )}
        {tab === "conversation" && (
          <ConversationCard store={store} settings={settings} />
        )}
...
function Field({
  ...
  return (
    <div className="dsh-layout-settings__field">
...
function CardHeading({
  ...
  return (
    <div className="dsh-layout-section-heading">
      <div className="dsh-layout-settings__label">
  ...
  return (
    <section className="dsh-layout-settings__card">
      <CardHeading
        icon={<RadiusIcon />}
        title="全局"
        hint="圆角、页面背景、弹窗尺寸与边距等跨区域选项；默认保持 DSH 原生。"
      />
        ...
        <div className="dsh-layout-range">
          ...
          {global.radius !== null && (
            <input
              ...
              value={global.radius}
              onChange={(event) =>
                setGlobal({ radius: Number(event.target.value) })
              }
            />
          )}
      ...
      <Field label="设置弹窗" path="global.dialog">
        <div className="dsh-layout-range">
          ...
          {global.dialog.width !== null && (
            <output>宽 {global.dialog.width}</output>
          )}
        </div>
      ...
      <Field label="手机侧边栏" path="global.narrow.sidebar">
        <div className="dsh-layout-segmented">
          ...
          <SegmentedButton
            ...
            onClick={() =>
              setGlobal({ narrow: { ...global.narrow, sidebar: "fullscreen" } })
            }
          >
            全屏覆盖
          </SegmentedButton>
  ...
  return (
    <section className="dsh-layout-settings__card">
      <CardHeading
        icon={<GlassIcon />}
        title="材质"
        hint="一张磨砂材质覆盖整个页面：侧边栏与内容区域共用同一层。"
      />
          ...
          <Field label="不透明度" path="material.opacity">
            <div className="dsh-layout-range">
              <input
                ...
                onChange={(event) =>
                  setMaterial({ opacity: Number(event.target.value) })
                }
              />
              <output>{material.opacity}%</output>
            </div>
          </Field>
          <Field label="模糊强度" path="material.blur">
            <div className="dsh-layout-range">
              <input
                ...
                value={material.blur}
                onChange={(event) =>
                  setMaterial({ blur: Number(event.target.value) })
                }
              />
              <output>{material.blur}px</output>
            </div>
          </Field>
          <Field label="饱和度" path="material.saturation">
            <div className="dsh-layout-range">
              <input
                ...
                onChange={(event) =>
                  setMaterial({ saturation: Number(event.target.value) })
                }
              />
              <output>{material.saturation}%</output>
            </div>
          </Field>
        </>
      )}
  ...
  return (
    <section className="dsh-layout-settings__card">
      <CardHeading
        icon={<WidthIcon />}
        title="对话"
        hint="阅读宽度、输入区与对话内容的呈现；每一项只做自己的事。"
      />
      <Group label="排版" />
