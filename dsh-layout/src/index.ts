...
export async function apply(ctx: Context): Promise<void> {
  const filePath = resolve(
    process.env.DSH_LAYOUT_FILE ??
      join(process.env.DSH_HOME ?? join(homedir(), ".dsh"), "dsh-layout.json"),
  );
  ...
  ctx.effect(
    () =>
      ctx.connection.rpc.handle(DSH_LAYOUT_CHANNEL, handle, {
        authority: "trusted-host",
      }),
    "dsh-layout: file persistence",
  );
}
...
function fail(error: string): RpcResult<unknown> {
  return {
    ok: false,
    error: { code: "internal", message: error, details: {} },
  };
}
