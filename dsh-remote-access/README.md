...
通过 GitHub Release 预构建包安装（已含构建产物，无需本机构建）。**用版本化路径**（路径带版本号、文件名不带——pnpm 缓存按 URL 区分，升级永不撞 integrity）：

```bash
dsh plugin --profile web add https://github.com/huangrx6/dsh-plugin/releases/download/0.1.0/dsh-remote-access.tgz
```

路径里的版本号以 [Releases 页](https://github.com/huangrx6/dsh-plugin/releases) 最新 tag 为准；**升级 = 换新 tag 号重跑**（路径变了，pnpm 缓存自动失效）。装完重启 dsh（或 `/reload`）。
